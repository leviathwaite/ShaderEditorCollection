...
void main() {
    ...
    float t = mod(time*max(grid.x,grid.y)+floor(1.0+time*u_mouse.y),totalCells);
    ...
    color = smoothstep(0.,.5+u_mouse.x/resolution.x*.5,color*color); // smooth
    color = step(0.5+u_mouse.x/resolution.x*0.5,color); // threshold
