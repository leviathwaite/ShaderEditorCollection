...
void main(void)
{
  ...
  uv *= 0.9;
  uv.x -= 0.5;
	vec4 faceTex = vec4(texture2D(Olive, uv));

  faceTex = vec4(texture2D(Robert, uv));
  // faceTex = vec4(texture2D(Liliana, uv));
  vec4 col = faceTex;
  ...
  // uv *= 1.5;
  // uv.y += 0.2;
  float c = Circle(vec2(uv.x, uv.y), 0.07, 0.25);
